...
async function parseAllFiles(files: string[]): Promise<ParsedFile[]> {
    ...
    for (const file of files) {
        ...
        parsedFiles.push({
            ...
            skillDescription:
                frontmatter.skillDescription || frontmatter.skill_description,
            skillParent: frontmatter.skillParent || frontmatter.skill_parent,
            skillInline:
                (frontmatter.skillInline || frontmatter.skill_inline) ===
                "true",
        });
    }
    return parsedFiles;
...
function discoverSkills(parsedFiles: ParsedFile[]): {
    parents: Parents;
    children: Children;
} {
...
async function surfaceParentGuide(
    ...
    if (parentInfo.file.skillInline) {
        return {
            inlineBody: stripFrontmatter(parentInfo.file.content).trim(),
            hasIndexReference: false,
        };
    }
...
async function buildSkill(
    ...
    const { inlineBody, hasIndexReference } = await surfaceParentGuide(
        parentInfo,
        refDir,
    );
    const { sitemap, subSkillsCount } = await buildSitemap(
        childSkills,
        hasIndexReference,
        refDir,
    );
...
async function packageArchive(): Promise<void> {
    ...
    console.log(
        "🎉 Skills packaging completed! Created dcc-skills.zip in .vitepress/dist/",
    );
}

async function main() {
    console.log(
        "Scanning markdown files recursively for parent and child skills...",
    );
    ...
    let skillsCount = 0;
    let subSkillsCount = 0;
    const sortedParents = [...parents.entries()].sort((a, b) =>
        a[0].localeCompare(b[0]),
    );
    for (const [parentId, parentInfo] of sortedParents) {
        subSkillsCount += await buildSkill(
            skillsDir,
            parentId,
            parentInfo,
    ...
    console.log(
        `🎉 Success! Compiled ${skillsCount} top-level skills with ${subSkillsCount} sub-skill references.`,
    );

    if (skillsCount === 0) {
        console.error(
            "❌ No skills were compiled. Check that markdown files declare skillName/skillDescription frontmatter.",
        );
        process.exit(1);
    }
